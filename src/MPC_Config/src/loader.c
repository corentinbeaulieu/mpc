						...
						if (_utils_strcasestarts_with(key, conf_name))
						{
							...
							if (!mpc_conf_root_config_get(double_conf_name))
							{
								_utils_verbose_output(0,
									"%s:%d: it is not needed to prefix keys with '%s_'\n",
									path,
									line,
									conf_name);
							}
	...
	else
	{
		...
		for (i = 0 ; i < section_count ; i++)
		{
			...
			_utils_verbose_output(1, "%s: loading section [%s] %d :\n%s\n", path, section_headers[i],
				strlen(section_start), section_start);

			if (strlen(section_start))
			{
				if (___mpc_conf_config_load_eq_list(conf_name, section_start, ".", can_create, path, line_starts[i],
					section_headers[i]))
				{
...
static inline int __mpc_conf_config_load_prefix(char *conf_name, char *path, int can_create)
{
	...
	if (_utils_is_dir(path))
	{
		...
		while ((elem = readdir(dir)) != NULL)
		{
			...
			{
				_utils_verbose_output(0,
					"when loading config for %s only regular files are expected for %s \n",
					conf_name,
					cpath);
				continue;
			}
...
int mpc_conf_config_load(char *conf_name)
{
	...
	if (__mpc_conf_config_load_prefix(conf_name,
		system,
		(rights == MPC_CONF_MOD_SYSTEM) || (rights == MPC_CONF_MOD_BOTH)))
	{
	...
	if (strlen(user) && _utils_is_file_or_dir(user))
	{
		if (__mpc_conf_config_load_prefix(conf_name,
			user,
			(rights == MPC_CONF_MOD_USER) || (rights == MPC_CONF_MOD_BOTH)))
		{
	...
	if (manual_conf)
	{
		...
		if (__mpc_conf_config_load_prefix(conf_name, manual_conf,
			(rights == MPC_CONF_MOD_USER) || (rights == MPC_CONF_MOD_BOTH)))
		{
