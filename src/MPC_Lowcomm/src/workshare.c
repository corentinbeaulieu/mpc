...
#include "thread.h"
	...
	void mpc_lowcomm_workshare_start(void (*func) (void *, long long, long long),
	                                 void *shareds,
	                                 long long lb,
	                                 long long ub,
	                                 long long incr,
	                                 ...
	                                 int nowait)
	{
		/* Entry function for #pragma ws for */
			...
			if (threshold_size > (ub - lb) / incr)
			{
				threshold_size = (ub - lb) / incr;
			}
			if (threshold_size <= 0)
			{
				threshold_size = 1;
	...
	static inline unsigned long long __get_chunk_and_execute_steal_from_end(mpc_workshare *ws_infos,
	                                                                        int is_stealing,
	                                                                        __attribute__(
																				(unused)) int is_in_collective)
	{
		/* Function called when stealing from end (WS_STEAL_FROM_END=1) */
		long long int ret = 0, cur_index, new_cur_index, chunk_size, reverse_index, new_reverse_index,
		              steal_chunk_size, not_stealing_chunk_size;
		int num_tasks;
		unsigned long long chunk_done = 0;
		int new_threshold_index = 0, not_stealing_threshold_index, anc_threshold_index,
		    stealers_threshold_index;
			...
			if (ws_infos->steal_scheduling_type == MPC_WORKSHARE_SCHEDULE_GUIDED)
			{
				while (ret == 0)
				{
					...
					if (ws_infos->scheduling_type == MPC_WORKSHARE_SCHEDULE_GUIDED)
					{
						...
						chunk_size = (ws_infos->incr
						              > 0) ? (reverse_index - cur_index - current_chunk_size)
						             / (ws_infos->incr * num_tasks) : cur_index - (reverse_index - current_chunk_size)
						             / (-ws_infos->incr * num_tasks);
					}
					else
					{
						chunk_size = (ws_infos->incr
						              > 0) ? (reverse_index - cur_index - ws_infos->chunk_size)
						             / (ws_infos->incr * num_tasks) : (cur_index - reverse_index
						                                               - ws_infos->chunk_size)
						             / (-ws_infos->incr * num_tasks);
					}
					...
					if ((reverse_index - cur_index - (chunk_size + not_stealing_chunk_size + ws_infos->chunk_size)
					     * ws_infos->incr < 0 && ws_infos->incr > 0)
					    || (reverse_index - cur_index - chunk_size * ws_infos->incr > 0 && ws_infos->incr < 0))
					{
						return 0;
					}
					...
					ret = (long long)OPA_cas_ptr(&(ws_infos->reverse_index),
						(void *)reverse_index,
						(void *)new_reverse_index);
					ret = (ret == reverse_index) ? 1 : 0;
					if (ret)
					{
						(long long)OPA_cas_ptr(&(ws_infos->current_chunk_size),
							(void *)current_chunk_size,
							(void *)chunk_size);
					}
				}
			}
			else
			{
				...
				while (ret == 0)
				{
					...
					else
					{
						...
						new_threshold_index = (new_reverse_index - ws_infos->lb - chunk_size
						                       * ws_infos->incr) * ws_infos->inv_threshold_size;
					...
					cur_index = (long long)OPA_load_ptr(&(ws_infos->cur_index));
					if ((ws_infos->incr > 0
					     && reverse_index - cur_index - (chunk_size + not_stealing_chunk_size) * ws_infos->incr < 0)
					    || (ws_infos->incr < 0
					        && cur_index - reverse_index
					        - ((chunk_size + not_stealing_chunk_size) * (-ws_infos->incr)) < 0))
					{
						return 0;
					}
					else
...
cas_ptr:
						...
						{
							int threshold_index = (reverse_index - ws_infos->lb - chunk_size * ws_infos->incr)
							                      * ws_infos->inv_threshold_size;

							if (threshold_index != new_threshold_index
							    && threshold_index != ws_infos->threshold_number - 1)
							{
		...
		else // not stealing
		{
			if (ws_infos->steal_scheduling_type == MPC_WORKSHARE_SCHEDULE_GUIDED)
			{
				if (ws_infos->scheduling_type == MPC_WORKSHARE_SCHEDULE_GUIDED)
				{
					while (ret == 0)
					{
						...
						long long current_chunk_size = (long long)OPA_load_ptr(&(ws_infos->current_chunk_size));
						chunk_size = (ws_infos->incr
						              > 0) ? (reverse_index - cur_index - (num_tasks - 1)
						                      * ws_infos->steal_chunk_size)
						             / (ws_infos->incr * num_tasks) : (cur_index - reverse_index - (num_tasks - 1)
						                                               * ws_infos->steal_chunk_size)
						             / (-ws_infos->incr * num_tasks);
						...
						ret = (long long)OPA_cas_ptr(&(ws_infos->current_chunk_size), (void *)current_chunk_size,
							(void *)chunk_size);
			...
			else
			{
				...
				else
				{
					...
					chunk_size          = ws_infos->chunk_size;
					new_cur_index       = cur_index + chunk_size * ws_infos->incr;
					new_threshold_index = (new_cur_index - ws_infos->lb + ws_infos->steal_chunk_size * ws_infos->incr)
					                      * ws_infos->inv_threshold_size;
					anc_threshold_index = (cur_index - ws_infos->lb + ws_infos->steal_chunk_size * ws_infos->incr)
					                      * ws_infos->inv_threshold_size;
					...
					{
						OPA_store_ptr(&(ws_infos->cur_index), (void *)new_cur_index);
						ws_infos->func(ws_infos->shareds,
							cur_index,
							new_cur_index);
						return 0;
					}
			...
			if ((reverse_index - cur_index - (chunk_size + steal_chunk_size + ws_infos->chunk_size) * ws_infos->incr
			     < 0 && ws_infos->incr > 0)
			    || (cur_index - reverse_index - (chunk_size + steal_chunk_size + ws_infos->chunk_size)
			        * -ws_infos->incr < 0 && ws_infos->incr < 0))
			{
				OPA_store_int(&(ws_infos->is_last_iter), 1);
				if ((long long)OPA_cas_ptr(&(ws_infos->reverse_index), (void *)reverse_index,
					(void *)(ws_infos->lb - ws_infos->incr)) != reverse_index)
				{
					while (OPA_load_int(&(ws_infos->nb_threads_stealing)) != 0)
					{
					}
	...
	static inline unsigned long long __get_chunk_and_execute_steal_from_start(mpc_workshare *ws_infos,
	                                                                          int is_stealing,
	                                                                          int is_in_collective)
	{
		...
		while (ret == 0)
		{
			...
			new_cur_index = cur_index + chunk_size * ws_infos->incr;
			if ((ws_infos->ub - cur_index - chunk_size * ws_infos->incr < 0 && ws_infos->incr > 0)
			    || (ws_infos->ub - cur_index - chunk_size * ws_infos->incr > 0 && ws_infos->incr < 0))
			{
		...
		if ((new_cur_index >= ws_infos->ub && ws_infos->incr > 0)
		    || (new_cur_index <= ws_infos->ub && ws_infos->incr < 0))
		{
			OPA_store_int(&(ws_infos->is_last_iter), 1);
		}
	...
	void __mpc_lowcomm_workshare_steal_wait_for_value(volatile int *data,
	                                                  int value,
	                                                  void (*func)(void *),
	                                                  void *arg,
	...
	void mpc_lowcomm_workshare_steal()
	{
			...
			{
				__mpc_lowcomm_workshare_steal_wait_for_value(NULL,
					0,
					NULL,
					NULL,
					mpc_thread_data_get()->workshare,
					mpc_common_get_local_task_rank());
			}
#endif
	}

	void mpc_lowcomm_workshare_steal_wait_for_value(__UNUSED__ volatile int *data,
	                                                __UNUSED__ int value,
	                                                __UNUSED__ void (*func)(void *),
	                                                __UNUSED__ void *arg)
	{
#ifdef MPC_Threads
			if (mpc_thread_data_get() && mpc_thread_data_get()->workshare)
	...
	void mpc_lowcomm_workshare_worker_steal(worker_workshare *worker_ws)
	{
			...
			if (worker_ws)
			{
				__mpc_lowcomm_workshare_steal_wait_for_value(NULL,
					...
					NULL,
					NULL,
					(mpc_workshare *)worker_ws->workshare,
					worker_ws->mpi_rank);
			}
#endif
	}
	...
	static int __choose_target_topological(mpc_workshare *workshare, int rank)
	{
		...
		int nb_cores_per_numa_node = ceil((float)mpc_common_get_local_task_count()
			/ mpc_topology_get_numa_node_count());
		...
		{
			if (i >= first_numa_rank && i <= last_numa_rank)
			{
				continue;
			}
			ws_infos = &workshare[i];
		...
		{
			if (i >= first_socket_rank && i < last_socket_rank)
			{
				continue;
			}
			ws_infos = &workshare[i];
	...
	static int __choose_target_strictly_topological(mpc_workshare *workshare, int rank)
	{
		...
		for (i = first_numa_rank; i < last_numa_rank; i++)
		{
			if (i >= mpc_common_get_local_task_count())
			{
				break;
			}
			ws_infos = &workshare[i];
