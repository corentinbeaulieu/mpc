...
static mpc_conf_config_type_t *__mpc_lowcomm_coll_conf_init(void)
{
	...
	mpc_conf_config_type_t *interleave = mpc_conf_config_type_init("interleave",
		PARAM(
			...
			MPC_CONF_INT,
			"Number of overallping SHM reduce"),
		PARAM("bcast",
			&__coll_conf.shm_bcast_interleave,
			MPC_CONF_INT,
	...
	mpc_conf_config_type_t *shm = mpc_conf_config_type_init("shm",
		PARAM("reducepipeline",
			...
			MPC_CONF_INT,
			"Number of blocks in the SHM reduce pipeline"),
	...
	mpc_conf_config_type_t *bcast = mpc_conf_config_type_init("bcast",
		PARAM("maxsize",
			...
			MPC_CONF_LONG_INT,
			"Maximum outgoing size per rank for a bcast defines arity (max_size / size)"),
		PARAM("arity", &__coll_conf.bcast_max_arity, MPC_CONF_INT, "Maximum arity  for a bcast"),
		PARAM("check",
			&__coll_conf.bcast_check_threshold,
			MPC_CONF_LONG_INT,
	...
	mpc_conf_config_type_t *allreduce = mpc_conf_config_type_init("allreduce",
		PARAM("maxsize",
			...
			MPC_CONF_LONG_INT,
			"Maximum outgoing size per rank for allreduce defines arity (max_size / size)"),
		PARAM("arity", &__coll_conf.allreduce_max_arity, MPC_CONF_INT, "Maximum arity  for allreduce"),
		PARAM("check",
			&__coll_conf.allreduce_check_threshold,
			MPC_CONF_LONG_INT,
			"Maximum size for checking messages immediately for copy"),
		PARAM("maxslots",
			&__coll_conf.allreduce_max_slots,
			MPC_CONF_INT,
	...
	mpc_conf_config_type_t *coll = mpc_conf_config_type_init("coll",
		PARAM("algo",
			...
			MPC_CONF_STRING,
			"Name of the collective module to use (simple, opt, hetero, noalloc)"),
...
void __mpc_lowcomm_coll_conf_validate(void)
{
	...
	{
		bad_parameter("mpcframework.lowcomm.coll.algo must be one of simple, opt, hetero, noalloc, had '%s'",
			__coll_conf.algorithm);
	}
}
...
{
	memset(__net_config.configs, 0,
		MPC_CONF_MAX_CONFIG_COUNT * sizeof(struct _mpc_lowcomm_config_struct_net_driver_config *));
...
static inline mpc_conf_config_type_t *__init_driver_portals(struct _mpc_lowcomm_config_struct_net_driver *driver)
{
	...
	mpc_conf_config_type_t *ret =
		mpc_conf_config_type_init("portals",
			...
			PARAM("maxmsgsize",
				&portals->max_msg_size,
				MPC_CONF_INT,
				"Max size of messages allowed to be sent."),
			PARAM("minfragsize",
				&portals->min_frag_size,
				MPC_CONF_LONG_INT,
				"Min size of fragments sent with multirail."),
...
static inline mpc_conf_config_type_t *___mpc_lowcomm_driver_instantiate_from_default(mpc_conf_config_type_t *config)
{
	...
	{
		bad_parameter("Config %s.%s should only contain a driver definition (expected MPC_CONF_TYPE not %s)",
			config->name,
			driver_dest->name,
...
static inline mpc_conf_config_type_t *__mpc_lowcomm_driver_conf_init()
{
	...
	mpc_conf_config_type_t *ret = mpc_conf_config_type_init("configs",
			...
			PARAM("mportalsconfigmpi",
			mportals,
			MPC_CONF_TYPE,
			"Default configuration for the Matching Portals4 Driver"),
#endif
#if defined(MPC_USE_OFI)
...
{
	memset(__net_config.rails,
		0,
		MPC_CONF_MAX_RAIL_COUNT * sizeof(struct _mpc_lowcomm_config_struct_net_rail *));
	...
	char *config)
{
	...
	mpc_conf_config_type_t *rail = mpc_conf_config_type_init(name,
		...
		PARAM("device",
			ret->device,
			MPC_CONF_STRING,
			"Name of the device to use can be a regular expression if starting with '!'"),
		...
		PARAM("maxifaces",
			&ret->max_ifaces,
			MPC_CONF_INT,
			"Maximum number of rails instances that can be used for multirail"),
		PARAM("offload", &ret->offload, MPC_CONF_INT, "Can this rail provide tag offloading capabilities."),
		PARAM("config",  ret->config,   MPC_CONF_STRING,
			"Name of the rail configuration to be used for this rail"),
		NULL);
...
static mpc_conf_config_type_t *___mpc_lowcomm_cli_conf_option_init(char *name, ...)
{
	...
	while (1)
	{
		...
		// prepare arguments
		char *rail_name_dup = malloc(sizeof(char) * MPC_CONF_STRING_SIZE);
		assume(rail_name_dup);
		snprintf(rail_name_dup,         MPC_CONF_STRING_SIZE, "%s",              rail_name);
		char param_name[MPC_CONF_STRING_SIZE];
		char param_config[MPC_CONF_STRING_SIZE];
		snprintf((char *)&param_name,   MPC_CONF_STRING_SIZE, "%d",              rail_nb);
...
static mpc_conf_config_type_t *__mpc_lowcomm_cli_conf_init(void)
{
	...
	mpc_conf_config_type_t *cliopt = mpc_conf_config_type_init("options",
		...
		PARAM("ptl",         ___mpc_lowcomm_cli_conf_option_init("ptl",         "tbsmmpi",     "portalsmpi",   NULL),                                 MPC_CONF_TYPE, "Portals"),
		PARAM("mptl",        ___mpc_lowcomm_cli_conf_option_init("mptl",        "mportalsmpi", NULL),                                                 MPC_CONF_TYPE, "Matching Portals. (without tbsm/shm)"),
#endif
#ifdef MPC_USE_OFI
		PARAM("verbsshm",    ___mpc_lowcomm_cli_conf_option_init("verbsshm",    "tbsmmpi",     "shmmpi",       "verbsofirail", NULL),                 MPC_CONF_TYPE, "Combination of Open Fabric Interface with Verbs and SHM"),
		PARAM("verbs",       ___mpc_lowcomm_cli_conf_option_init("verbs",       "tbsmmpi",     "verbsofirail", NULL),                                 MPC_CONF_TYPE, "Open Fabric Interface with Verbs"),
		PARAM("tcpshm",      ___mpc_lowcomm_cli_conf_option_init("tcpshm",      "tbsmmpi",     "shmmpi",       "tcpofirail",   NULL),                 MPC_CONF_TYPE, "Combination of Open Fabric Interface with TCP and SHM"),
		PARAM("tcp",         ___mpc_lowcomm_cli_conf_option_init("tcp",         "tbsmmpi",     "tcpofirail",   NULL),                                 MPC_CONF_TYPE, "Open Fabirc Interface with TCP"),
#else
		PARAM("tcpshm",      ___mpc_lowcomm_cli_conf_option_init("tcpshm",      "tbsmmpi",     "shmmpi",       "tcpmpi",       NULL),                 MPC_CONF_TYPE, "Combination of TCP and TBSM and SHM"),
		PARAM("tcp",         ___mpc_lowcomm_cli_conf_option_init("tcp",         "tbsmmpi",     "tcpmpi",       NULL),                                 MPC_CONF_TYPE, "Combination of TCP and TBSM"),
#endif
	NULL);
...
{
#if MPC_USE_OFI && MPC_USE_PORTALS
		snprintf(__net_config.cli_default_network, MPC_CONF_STRING_SIZE, "ptlverbsshm");
#elif MPC_USE_PORTALS
		snprintf(__net_config.cli_default_network, MPC_CONF_STRING_SIZE, "ptlshm");
#elif MPC_USE_OFI
		snprintf(__net_config.cli_default_network, MPC_CONF_STRING_SIZE, "verbsshm");
#else
...
static inline void ___mpc_lowcomm_cli_option_validate(mpc_conf_config_type_elem_t *opt)
{
	...
	{
		bad_parameter("mpcframework.lowcomm.networking.cli.options.%s erroneous should be MPC_CONF_TYPE not ",
			opt->name,
			mpc_conf_type_name(opt->type));
	}
...
static inline mpc_conf_config_type_t *__mpc_lowcomm_protocol_conf_init(void)
{
	...
	mpc_conf_config_type_t *ret = mpc_conf_config_type_init("protocol",
		...
		PARAM("maxmmuentries",
			&proto->max_mmu_entries,
			MPC_CONF_INT,
			"Maximum number of entries in the pinning cache"),
		PARAM("maxmmusize",
			&proto->mmu_max_size,
			MPC_CONF_LONG_INT,
			"Maximum size of the pinning cache"),
		NULL);
