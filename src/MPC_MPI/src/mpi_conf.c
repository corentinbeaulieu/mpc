...
static inline void __nbc_check(void)
{
	...
	{
		bad_parameter(
			"When loading mpcframework.mpi.nbc.progress.bindfunc = %s the function could not be resolved with dlsym",
			nbc->thread_bind_function_name);
	}
}
...
mpc_conf_config_type_t *__init_nbc_config(void)
{
	...
	mpc_conf_config_type_t *egreq = mpc_conf_config_type_init("egreq",
		...
		PARAM("gather",  &nbc->use_egreq_gather,  MPC_CONF_BOOL, "Enable EGREQ Barrier"),
...
mpc_conf_config_type_t *_mpc_mpi_config_coll_array_conf(struct _mpc_mpi_config_coll_array *coll)
{
	mpc_conf_config_type_t *ret = mpc_conf_config_type_init("pointers",
		...
		PARAM("alltoall",           coll->alltoall_name,             MPC_CONF_STRING, "alltoall function pointer"),
		PARAM("alltoallv",          coll->alltoallv_name,            MPC_CONF_STRING, "alltoallv function pointer"),
		PARAM("alltoallw",          coll->alltoallw_name,            MPC_CONF_STRING, "alltoallw function pointer"),
		PARAM("barrier",            coll->barrier_name,              MPC_CONF_STRING, "barrier function pointer"),
		PARAM("bcast",              coll->bcast_name,                MPC_CONF_STRING, "bcast function pointer"),
		PARAM("exscan",             coll->exscan_name,               MPC_CONF_STRING, "exscan function pointer"),
		PARAM("gather",             coll->gather_name,               MPC_CONF_STRING, "gather function pointer"),
		PARAM("gatherv",            coll->gatherv_name,              MPC_CONF_STRING, "gatherv function pointer"),
		PARAM("scan",               coll->scan_name,                 MPC_CONF_STRING, "scan function pointer"),
		PARAM("scatter",            coll->scatter_name,              MPC_CONF_STRING, "scatter function pointer"),
		PARAM("scatterv",           coll->scatterv_name,             MPC_CONF_STRING, "scatterv function pointer"),
		PARAM("reduce",             coll->reduce_name,               MPC_CONF_STRING, "Reduce function pointer"),
		PARAM("reducescatter",      coll->reduce_scatter_name,       MPC_CONF_STRING,
			"Reduce scatter function pointer"),
		PARAM("reducescatterblock", coll->reduce_scatter_block_name, MPC_CONF_STRING,
...
static inline void _mpc_mpi_config_coll_toggle_topo_full(char const *toggle)
{
	...
	else
	{
		...
		{
			bad_parameter(
				"Invalid toggle value '%s' in global topological-algorithm toggle, expected (on, off, true, false, auto)",
				toggle);
		}
	}
}
...
mpc_conf_config_type_t *__init_coll_config(void)
{
	...
	mpc_conf_config_type_t *shm = mpc_conf_config_type_init(
		...
		PARAM("pointers",             _mpc_mpi_config_coll_array_conf(&__mpc_mpi_config.coll_intracomm_shm),
	...
	mpc_conf_config_type_t *intracomm = mpc_conf_config_type_init("intracomm",
		...
		PARAM("pointers",         _mpc_mpi_config_coll_array_conf(&__mpc_mpi_config.coll_intracomm),
			MPC_CONF_TYPE,
			"Function pointers for intracomm collectives"),
		PARAM("algorithmpointers",
			_mpc_mpi_config_coll_algorithm_array_conf(&__mpc_mpi_config.coll_algorithm_intracomm),
			MPC_CONF_TYPE,
