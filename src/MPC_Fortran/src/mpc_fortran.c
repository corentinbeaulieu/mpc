...
void pmpi_accumulate_(const void *origin_addr,
                      int *origin_count,
...
void pmpi_accumulate__(const void *origin_addr,
                       int *origin_count,
                       MPI_Fint *origin_datatype,
                       int *target_rank,
                       MPI_Aint *target_disp,
...
void pmpi_add_error_string_(int *errorcode,
                            const char *string CHAR_MIXED(size_string),
...
void pmpi_add_error_string__(int *errorcode, const char *string CHAR_MIXED(size_string),
                             int *ierror CHAR_END(size_string))
{
	/* MPI_Add_error_string */
	char *tmp_string = NULL, *ptr_string = NULL;
...
void pmpi_allgather_(const void *sendbuf,
                     int *sendcount,
                     MPI_Fint *sendtype,
                     void *recvbuf,
                     int *recvcount,
...
void pmpi_allgather__(const void *sendbuf,
                      int *sendcount,
                      MPI_Fint *sendtype,
                      void *recvbuf,
                      int *recvcount,
...
void pmpi_allgather_init_(const void *sendbuf,
                          int *sendcount,
                          MPI_Fint *sendtype,
                          void *recvbuf,
                          int *recvcount,
                          ...
                          int *ierror)
{
	...
	*ierror = MPI_Allgather_init(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		*recvcount,
...
void pmpi_allgather_init__(const void *sendbuf,
                           int *sendcount,
                           MPI_Fint *sendtype,
                           void *recvbuf,
                           int *recvcount,
...
void pmpi_allgatherv_(const void *sendbuf,
                      int *sendcount,
                      MPI_Fint *sendtype,
                      void *recvbuf,
...
void pmpi_allgatherv__(const void *sendbuf,
                       int *sendcount,
                       MPI_Fint *sendtype,
                       void *recvbuf,
                       const int recvcounts[],
...
void pmpi_allgatherv_init_(const void *sendbuf,
                           int *sendcount,
                           MPI_Fint *sendtype,
                           void *recvbuf,
                           const int recvcounts[],
                           ...
                           int *ierror)
{
	...
	*ierror = MPI_Allgatherv_init(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		recvcounts,
...
void pmpi_allgatherv_init__(const void *sendbuf,
                            int *sendcount,
                            MPI_Fint *sendtype,
                            void *recvbuf,
                            const int recvcounts[],
...
void pmpi_allreduce_(const void *sendbuf,
                     void *recvbuf,
                     int *count,
                     MPI_Fint *datatype,
                     MPI_Fint *op,
...
void pmpi_allreduce__(const void *sendbuf,
                      void *recvbuf,
                      int *count,
                      MPI_Fint *datatype,
                      MPI_Fint *op,
...
void pmpi_allreduce_init_(const void *sendbuf,
                          void *recvbuf,
                          int *count,
                          MPI_Fint *datatype,
                          MPI_Fint *op,
...
void pmpi_allreduce_init__(const void *sendbuf,
                           void *recvbuf,
                           int *count,
                           MPI_Fint *datatype,
                           MPI_Fint *op,
...
void pmpi_alltoall_(const void *sendbuf,
                    int *sendcount,
                    MPI_Fint *sendtype,
                    void *recvbuf,
                    int *recvcount,
...
void pmpi_alltoall__(const void *sendbuf,
                     int *sendcount,
                     MPI_Fint *sendtype,
                     void *recvbuf,
                     int *recvcount,
...
void pmpi_alltoall_init_(const void *sendbuf,
                         int *sendcount,
                         MPI_Fint *sendtype,
                         void *recvbuf,
                         int *recvcount,
                         ...
                         int *ierror)
{
	...
	*ierror = MPI_Alltoall_init(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		*recvcount,
...
void pmpi_alltoall_init__(const void *sendbuf,
                          int *sendcount,
                          MPI_Fint *sendtype,
                          void *recvbuf,
                          int *recvcount,
...
void pmpi_alltoallv_(const void *sendbuf,
...
void pmpi_alltoallv__(const void *sendbuf,
                      const int sendcounts[],
                      const int sdispls[],
                      MPI_Fint *sendtype,
                      void *recvbuf,
...
void pmpi_alltoallv_init_(const void *sendbuf,
                          const int sendcounts[],
                          const int sdispls[],
                          MPI_Fint *sendtype,
                          void *recvbuf,
                          ...
                          int *ierror)
{
	...
	*ierror = MPI_Alltoallv_init(sendbuf,
		sendcounts,
		sdispls,
		c_sendtype,
		recvbuf,
...
void pmpi_alltoallv_init__(const void *sendbuf,
                           const int sendcounts[],
                           const int sdispls[],
                           MPI_Fint *sendtype,
                           void *recvbuf,
...
void pmpi_alltoallw_(const void *sendbuf,
                     const int sendcounts[],
                     const int sdispls[],
                     ...
                     int *ierror)
{
	...
	*ierror = MPI_Alltoallw(sendbuf, sendcounts, sdispls, c_sendtypes, recvbuf, recvcounts,
...
void pmpi_alltoallw__(const void *sendbuf,
                      const int sendcounts[],
                      const int sdispls[],
                      const MPI_Fint sendtypes[],
                      void *recvbuf,
                      ...
                      int *ierror)
{
	...
	*ierror = MPI_Alltoallw(sendbuf, sendcounts, sdispls, c_sendtypes, recvbuf, recvcounts, rdispls, c_recvtypes,
		c_comm);
	sctk_free(c_sendtypes);
	sctk_free(c_recvtypes);
}
...
void pmpi_alltoallw_init_(const void *sendbuf,
                          const int sendcounts[],
                          const int sdispls[],
                          const MPI_Fint sendtypes[],
                          void *recvbuf,
                          ...
                          int *ierror)
{
	...
	*ierror = MPI_Alltoallw_init(sendbuf,
		sendcounts,
		sdispls,
		c_sendtypes,
		recvbuf,
...
void pmpi_alltoallw_init__(const void *sendbuf,
                           const int sendcounts[],
                           const int sdispls[],
                           const MPI_Fint sendtypes[],
                           void *recvbuf,
...
void pmpi_bcast_init_(void *buffer,
                      int *count,
                      MPI_Fint *datatype,
                      int *root,
                      MPI_Fint *comm,
...
void pmpi_bcast_init__(void *buffer,
                       int *count,
                       MPI_Fint *datatype,
                       int *root,
                       MPI_Fint *comm,
...
void pmpi_bsend_init_(const void *buf,
                      int *count,
                      MPI_Fint *datatype,
                      int *dest,
                      int *tag,
...
void pmpi_bsend_init__(const void *buf,
                       int *count,
                       MPI_Fint *datatype,
                       int *dest,
                       int *tag,
...
void pmpi_cart_create_(MPI_Fint *comm_old,
                       int *ndims,
                       const int dims[],
                       const int periods[],
                       int *reorder,
...
void pmpi_cart_create__(MPI_Fint *comm_old,
                        int *ndims,
                        const int dims[],
                        const int periods[],
                        int *reorder,
...
void pmpi_comm_accept_(const char *port_name CHAR_MIXED(size_port_name),
                       MPI_Fint *info,
                       int *root,
                       MPI_Fint *comm,
                       MPI_Fint *newcomm,
...
void pmpi_comm_accept__(const char *port_name CHAR_MIXED(size_port_name),
                        MPI_Fint *info,
                        int *root,
                        MPI_Fint *comm,
                        MPI_Fint *newcomm,
...
void pmpi_comm_connect_(const char *port_name CHAR_MIXED(size_port_name),
                        MPI_Fint *info,
                        int *root,
                        MPI_Fint *comm,
                        MPI_Fint *newcomm,
...
void pmpi_comm_connect__(const char *port_name CHAR_MIXED(size_port_name),
                         MPI_Fint *info,
                         int *root,
                         MPI_Fint *comm,
                         MPI_Fint *newcomm,
...
void pmpi_comm_create_keyval_(MPI_Comm_copy_attr_function *comm_copy_attr_fn,
...
void pmpi_comm_create_keyval__(MPI_Comm_copy_attr_function *comm_copy_attr_fn,
                               MPI_Comm_delete_attr_function *comm_delete_attr_fn,
                               int *comm_keyval,
                               void *extra_state,
                               int *ierror)
...
void pmpi_comm_get_name__(MPI_Fint *comm,
                          char *comm_name CHAR_MIXED(size_comm_name),
                          int *resultlen,
                          int *ierror CHAR_END(size_comm_name))
{
...
void pmpi_comm_set_name_(MPI_Fint *comm, const char *comm_name CHAR_MIXED(size_comm_name),
                         int *ierror CHAR_END(size_comm_name))
{
	/* MPI_Comm_set_name */
	MPI_Comm c_comm = PMPI_Comm_f2c(*comm);
...
void pmpi_comm_set_name__(MPI_Fint *comm, const char *comm_name CHAR_MIXED(size_comm_name),
                          int *ierror CHAR_END(size_comm_name))
{
	/* MPI_Comm_set_name */
	MPI_Comm c_comm = PMPI_Comm_f2c(*comm);
...
void pmpi_comm_spawn_(const char *command CHAR_MIXED(size_command),
                      char *argv[],
                      int *maxprocs,
                      MPI_Fint *info,
                      int *root,
                      ...
                      int *ierror CHAR_END(size_command))
{
	/* MPI_Comm_spawn */
...
void pmpi_comm_spawn__(const char *command CHAR_MIXED(size_command),
                       char *argv[],
                       int *maxprocs,
                       MPI_Fint *info,
                       int *root,
...
void pmpi_comm_spawn_multiple_(int *count,
                               ...
                               int *ierror)
{
	...
	*ierror = MPI_Comm_spawn_multiple(*count,
		array_of_commands,
		array_of_argv,
		array_of_maxprocs,
		c_array_of_info,
...
void pmpi_comm_spawn_multiple__(int *count,
                                char *array_of_commands[],
                                char **array_of_argv[],
                                const int array_of_maxprocs[],
                                const MPI_Fint array_of_info[],
...
void pmpi_compare_and_swap_(const void *origin_addr,
                            ...
                            int *ierror)
{
	...
	*ierror = MPI_Compare_and_swap(origin_addr, compare_addr, result_addr, c_datatype, *target_rank, *target_disp,
		c_win);
}

#pragma weak mpi_compare_and_swap__ = pmpi_compare_and_swap__
void pmpi_compare_and_swap__(const void *origin_addr,
                             const void *compare_addr,
                             void *result_addr,
                             MPI_Fint *datatype,
                             int *target_rank,
...
void pmpi_dist_graph_create_(MPI_Fint *comm_old,
                             int *n,
                             const int sources[],
                             const int degrees[],
                             ...
                             int *ierror)
{
	...
	*ierror = MPI_Dist_graph_create(c_comm_old,
		*n,
		sources,
		degrees,
		destinations,
...
void pmpi_dist_graph_create__(MPI_Fint *comm_old,
                              int *n,
                              const int sources[],
                              const int degrees[],
                              const int destinations[],
...
void pmpi_dist_graph_create_adjacent_(MPI_Fint *comm_old,
                                      int *indegree,
                                      const int sources[],
                                      ...
                                      int *ierror)
{
	...
	*ierror = MPI_Dist_graph_create_adjacent(c_comm_old,
		*indegree,
		sources,
		sourceweights,
		*outdegree,
...
void pmpi_dist_graph_create_adjacent__(MPI_Fint *comm_old,
                                       int *indegree,
                                       const int sources[],
                                       const int sourceweights[],
                                       int *outdegree,
...
void pmpi_dist_graph_neighbors_(MPI_Fint *comm,
                                int *maxindegree,
                                int sources[],
                                int sourceweights[],
                                int *maxoutdegree,
                                ...
                                int *ierror)
{
	...
	*ierror = MPI_Dist_graph_neighbors(c_comm,
		*maxindegree,
		sources,
		sourceweights,
		*maxoutdegree,
...
void pmpi_dist_graph_neighbors__(MPI_Fint *comm,
                                 int *maxindegree,
                                 int sources[],
                                 int sourceweights[],
                                 int *maxoutdegree,
...
void pmpi_error_string_(int *errorcode, char *string CHAR_MIXED(size_string), int *resultlen,
                        int *ierror CHAR_END(size_string))
{
	/* MPI_Error_string */
...
void pmpi_error_string__(int *errorcode, char *string CHAR_MIXED(size_string), int *resultlen,
                         int *ierror CHAR_END(size_string))
{
	/* MPI_Error_string */
...
void pmpi_exscan_(const void *sendbuf,
                  void *recvbuf,
                  int *count,
                  MPI_Fint *datatype,
                  MPI_Fint *op,
...
void pmpi_exscan__(const void *sendbuf,
                   void *recvbuf,
                   int *count,
                   MPI_Fint *datatype,
                   MPI_Fint *op,
...
void pmpi_exscan_init_(const void *sendbuf,
                       void *recvbuf,
                       int *count,
                       MPI_Fint *datatype,
                       MPI_Fint *op,
...
void pmpi_exscan_init__(const void *sendbuf,
                        void *recvbuf,
                        int *count,
                        MPI_Fint *datatype,
                        MPI_Fint *op,
...
void pmpi_fetch_and_op_(const void *origin_addr,
                        void *result_addr,
                        MPI_Fint *datatype,
                        int *target_rank,
                        MPI_Aint *target_disp,
...
void pmpi_fetch_and_op__(const void *origin_addr,
                         void *result_addr,
                         MPI_Fint *datatype,
                         int *target_rank,
                         MPI_Aint *target_disp,
...
void pmpi_gather_(const void *sendbuf,
                  int *sendcount,
                  MPI_Fint *sendtype,
                  void *recvbuf,
                  int *recvcount,
...
void pmpi_gather__(const void *sendbuf,
                   int *sendcount,
                   MPI_Fint *sendtype,
                   void *recvbuf,
                   int *recvcount,
...
void pmpi_gather_init_(const void *sendbuf,
                       int *sendcount,
                       MPI_Fint *sendtype,
                       void *recvbuf,
                       int *recvcount,
                       ...
                       int *ierror)
{
	...
	*ierror = MPI_Gather_init(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		*recvcount,
...
void pmpi_gather_init__(const void *sendbuf,
                        int *sendcount,
                        MPI_Fint *sendtype,
                        void *recvbuf,
                        int *recvcount,
...
void pmpi_gatherv_(const void *sendbuf,
                   int *sendcount,
                   MPI_Fint *sendtype,
                   void *recvbuf,
                   const int recvcounts[],
...
void pmpi_gatherv__(const void *sendbuf,
                    int *sendcount,
                    MPI_Fint *sendtype,
                    void *recvbuf,
                    const int recvcounts[],
...
void pmpi_gatherv_init_(const void *sendbuf,
                        int *sendcount,
                        MPI_Fint *sendtype,
                        void *recvbuf,
                        const int recvcounts[],
                        ...
                        int *ierror)
{
	...
	*ierror = MPI_Gatherv_init(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		recvcounts,
...
void pmpi_gatherv_init__(const void *sendbuf,
                         int *sendcount,
                         MPI_Fint *sendtype,
                         void *recvbuf,
                         const int recvcounts[],
...
void pmpi_get_(void *origin_addr,
               int *origin_count,
               MPI_Fint *origin_datatype,
               int *target_rank,
               MPI_Aint *target_disp,
...
void pmpi_get__(void *origin_addr,
                int *origin_count,
                MPI_Fint *origin_datatype,
                int *target_rank,
                MPI_Aint *target_disp,
...
void pmpi_get_accumulate_(const void *origin_addr,
                          int *origin_count,
                          MPI_Fint *origin_datatype,
                          void *result_addr,
                          int *result_count,
                          ...
                          int *ierror)
{
	...
	*ierror = MPI_Get_accumulate(origin_addr,
		*origin_count,
		c_origin_datatype,
		result_addr,
		*result_count,
...
void pmpi_get_accumulate__(const void *origin_addr,
                           int *origin_count,
                           MPI_Fint *origin_datatype,
                           void *result_addr,
                           int *result_count,
...
void pmpi_get_library_version_(char *version CHAR_MIXED(size_version),
                               int *resultlen,
                               int *ierror CHAR_END(size_version))
{
...
void pmpi_get_library_version__(char *version CHAR_MIXED(size_version), int *resultlen,
                                int *ierror CHAR_END(size_version))
{
	/* MPI_Get_library_version */
...
void pmpi_graph_create__(MPI_Fint *comm_old,
                         int *nnodes,
                         const int index[],
                         const int edges[],
                         int *reorder,
...
void pmpi_grequest_start_(MPI_Grequest_query_function *query_fn,
...
void pmpi_grequest_start__(MPI_Grequest_query_function *query_fn,
                           MPI_Grequest_free_function *free_fn,
                           MPI_Grequest_cancel_function *cancel_fn,
                           void *extra_state,
                           MPI_Fint *request,
...
void pmpi_group_translate_ranks_(MPI_Fint *group1,
                                 int *n,
                                 const int ranks1[],
                                 MPI_Fint *group2,
                                 int ranks2[],
...
void pmpi_group_translate_ranks__(MPI_Fint *group1,
                                  int *n,
                                  const int ranks1[],
                                  MPI_Fint *group2,
                                  int ranks2[],
...
void pmpi_iallgather_(const void *sendbuf,
                      int *sendcount,
                      MPI_Fint *sendtype,
                      void *recvbuf,
                      int *recvcount,
...
void pmpi_iallgather__(const void *sendbuf,
                       int *sendcount,
                       MPI_Fint *sendtype,
                       void *recvbuf,
                       int *recvcount,
...
void pmpi_iallgatherv_(const void *sendbuf,
                       int *sendcount,
                       MPI_Fint *sendtype,
                       void *recvbuf,
                       const int recvcounts[],
                       ...
                       int *ierror)
{
	...
	*ierror = MPI_Iallgatherv(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		recvcounts,
...
void pmpi_iallgatherv__(const void *sendbuf,
                        int *sendcount,
                        MPI_Fint *sendtype,
                        void *recvbuf,
                        const int recvcounts[],
...
void pmpi_iallreduce_(const void *sendbuf,
                      void *recvbuf,
                      int *count,
                      MPI_Fint *datatype,
                      MPI_Fint *op,
...
void pmpi_iallreduce__(const void *sendbuf,
                       void *recvbuf,
                       int *count,
                       MPI_Fint *datatype,
                       MPI_Fint *op,
...
void pmpi_ialltoall_(const void *sendbuf,
                     int *sendcount,
                     MPI_Fint *sendtype,
                     void *recvbuf,
                     int *recvcount,
...
void pmpi_ialltoall__(const void *sendbuf,
                      int *sendcount,
                      MPI_Fint *sendtype,
                      void *recvbuf,
                      int *recvcount,
...
void pmpi_ialltoallv_(const void *sendbuf,
                      const int sendcounts[],
                      const int sdispls[],
                      MPI_Fint *sendtype,
                      void *recvbuf,
...
void pmpi_ialltoallv__(const void *sendbuf,
                       const int sendcounts[],
                       const int sdispls[],
                       MPI_Fint *sendtype,
                       void *recvbuf,
...
void pmpi_ialltoallw_(const void *sendbuf,
                      const int sendcounts[],
                      const int sdispls[],
                      const MPI_Fint sendtypes[],
                      void *recvbuf,
...
void pmpi_ialltoallw__(const void *sendbuf,
                       const int sendcounts[],
                       const int sdispls[],
                       const MPI_Fint sendtypes[],
                       void *recvbuf,
...
void pmpi_ibcast__(void *buffer,
                   int *count,
                   MPI_Fint *datatype,
                   int *root,
                   MPI_Fint *comm,
...
void pmpi_ibsend_(const void *buf,
                  int *count,
                  MPI_Fint *datatype,
                  int *dest,
                  int *tag,
...
void pmpi_ibsend__(const void *buf,
                   int *count,
                   MPI_Fint *datatype,
                   int *dest,
                   int *tag,
...
void pmpi_iexscan_(const void *sendbuf,
                   void *recvbuf,
                   int *count,
                   MPI_Fint *datatype,
                   MPI_Fint *op,
...
void pmpi_iexscan__(const void *sendbuf,
                    void *recvbuf,
                    int *count,
                    MPI_Fint *datatype,
                    MPI_Fint *op,
...
void pmpi_igather_(const void *sendbuf,
                   int *sendcount,
                   MPI_Fint *sendtype,
                   void *recvbuf,
                   int *recvcount,
...
void pmpi_igather__(const void *sendbuf,
                    int *sendcount,
                    MPI_Fint *sendtype,
                    void *recvbuf,
                    int *recvcount,
...
void pmpi_igatherv_(const void *sendbuf,
                    int *sendcount,
                    MPI_Fint *sendtype,
                    void *recvbuf,
                    const int recvcounts[],
...
void pmpi_igatherv__(const void *sendbuf,
                     int *sendcount,
                     MPI_Fint *sendtype,
                     void *recvbuf,
                     const int recvcounts[],
...
void pmpi_improbe__(int *source,
                    int *tag,
                    MPI_Fint *comm,
                    int *flag,
                    MPI_Message *message,
...
void pmpi_ineighbor_allgather_(const void *sendbuf,
                               int *sendcount,
                               MPI_Fint *sendtype,
                               void *recvbuf,
                               int *recvcount,
                               ...
                               int *ierror)
{
	...
	*ierror = MPI_Ineighbor_allgather(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		*recvcount,
...
void pmpi_ineighbor_allgather__(const void *sendbuf,
                                int *sendcount,
                                MPI_Fint *sendtype,
                                void *recvbuf,
                                int *recvcount,
...
void pmpi_ineighbor_allgatherv_(const void *sendbuf,
                                int *sendcount,
                                MPI_Fint *sendtype,
                                void *recvbuf,
                                const int recvcounts[],
                                ...
                                int *ierror)
{
	...
	*ierror = MPI_Ineighbor_allgatherv(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		recvcounts,
...
void pmpi_ineighbor_allgatherv__(const void *sendbuf,
                                 int *sendcount,
                                 MPI_Fint *sendtype,
                                 void *recvbuf,
                                 const int recvcounts[],
...
void pmpi_ineighbor_alltoall_(const void *sendbuf,
                              int *sendcount,
                              MPI_Fint *sendtype,
                              void *recvbuf,
                              int *recvcount,
                              ...
                              int *ierror)
{
	...
	*ierror = MPI_Ineighbor_alltoall(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		*recvcount,
...
void pmpi_ineighbor_alltoall__(const void *sendbuf,
                               int *sendcount,
                               MPI_Fint *sendtype,
                               void *recvbuf,
                               int *recvcount,
...
void pmpi_ineighbor_alltoallv_(const void *sendbuf,
                               const int sendcounts[],
                               const int sdispls[],
                               MPI_Fint *sendtype,
                               void *recvbuf,
                               ...
                               int *ierror)
{
	...
	*ierror = MPI_Ineighbor_alltoallv(sendbuf,
		sendcounts,
		sdispls,
		c_sendtype,
		recvbuf,
...
void pmpi_ineighbor_alltoallv__(const void *sendbuf,
                                const int sendcounts[],
                                const int sdispls[],
                                MPI_Fint *sendtype,
                                void *recvbuf,
...
void pmpi_ineighbor_alltoallw_(const void *sendbuf,
                               const int sendcounts[],
                               ...
                               int *ierror)
{
	...
	*ierror = MPI_Ineighbor_alltoallw(sendbuf,
		sendcounts,
		sdispls,
		c_sendtypes,
		recvbuf,
...
void pmpi_ineighbor_alltoallw__(const void *sendbuf,
                                const int sendcounts[],
                                const MPI_Aint sdispls[],
                                const MPI_Fint sendtypes[],
                                void *recvbuf,
...
void pmpi_info_get_(MPI_Fint *info, const char *key CHAR_MIXED(size_key), int *valuelen,
                    char *value CHAR_MIXED(size_value), int *flag, int *ierror CHAR_END(size_key)CHAR_END(size_value))
{
	/* MPI_Info_get */
	MPI_Info c_info = PMPI_Info_f2c(*info);
	char *   tmp_key = NULL, *ptr_key = NULL;
...
void pmpi_info_get__(MPI_Fint *info, const char *key CHAR_MIXED(size_key), int *valuelen,
                     char *value CHAR_MIXED(size_value), int *flag, int *ierror CHAR_END(size_key)CHAR_END(size_value))
{
	/* MPI_Info_get */
	MPI_Info c_info = PMPI_Info_f2c(*info);
...
void pmpi_info_get_string_(MPI_Fint *info, const char *key CHAR_MIXED(size_key), int *buflen,
                           char *value CHAR_MIXED(size_value), int *flag,
...
void pmpi_info_get_string__(MPI_Fint *info, const char *key CHAR_MIXED(size_key), int *buflen,
                            char *value CHAR_MIXED(size_value), int *flag,
                            int *ierror CHAR_END(size_key)CHAR_END(size_value))
{
	/* MPI_Info_get_string */
...
void pmpi_info_get_valuelen_(MPI_Fint *info,
                             const char *key CHAR_MIXED(size_key),
                             int *valuelen,
                             int *flag,
...
void pmpi_info_get_valuelen__(MPI_Fint *info,
                              const char *key CHAR_MIXED(size_key),
                              int *valuelen,
                              int *flag,
                              int *ierror CHAR_END(size_key))
...
void pmpi_info_set_(MPI_Fint *info,
                    const char *key CHAR_MIXED(size_key),
                    const char *value CHAR_MIXED(size_value),
                    int *ierror CHAR_END(size_key)CHAR_END(size_value))
{
	/* MPI_Info_set */
	MPI_Info c_info = PMPI_Info_f2c(*info);
...
void pmpi_info_set__(MPI_Fint *info,
                     const char *key CHAR_MIXED(size_key),
                     const char *value CHAR_MIXED(size_value),
                     int *ierror CHAR_END(size_key)CHAR_END(size_value))
{
...
void pmpi_ireduce_(const void *sendbuf,
                   void *recvbuf,
                   int *count,
                   MPI_Fint *datatype,
                   MPI_Fint *op,
...
void pmpi_ireduce__(const void *sendbuf,
                    void *recvbuf,
                    int *count,
                    MPI_Fint *datatype,
                    MPI_Fint *op,
...
void pmpi_ireduce_scatter_(const void *sendbuf,
                           void *recvbuf,
                           const int recvcounts[],
                           MPI_Fint *datatype,
                           MPI_Fint *op,
...
void pmpi_ireduce_scatter__(const void *sendbuf,
                            void *recvbuf,
                            const int recvcounts[],
                            MPI_Fint *datatype,
                            MPI_Fint *op,
...
void pmpi_ireduce_scatter_block_(const void *sendbuf,
                                 void *recvbuf,
                                 int *recvcount,
                                 MPI_Fint *datatype,
                                 MPI_Fint *op,
...
void pmpi_ireduce_scatter_block__(const void *sendbuf,
                                  void *recvbuf,
                                  int *recvcount,
                                  MPI_Fint *datatype,
                                  MPI_Fint *op,
...
void pmpi_irsend_(const void *buf,
                  int *count,
                  MPI_Fint *datatype,
                  int *dest,
                  int *tag,
...
void pmpi_irsend__(const void *buf,
                   int *count,
                   MPI_Fint *datatype,
                   int *dest,
                   int *tag,
...
void pmpi_iscan_(const void *sendbuf,
                 void *recvbuf,
                 int *count,
                 MPI_Fint *datatype,
                 MPI_Fint *op,
...
void pmpi_iscan__(const void *sendbuf,
                  void *recvbuf,
                  int *count,
                  MPI_Fint *datatype,
                  MPI_Fint *op,
...
void pmpi_iscatter_(const void *sendbuf,
                    int *sendcount,
                    MPI_Fint *sendtype,
                    void *recvbuf,
                    int *recvcount,
                    ...
                    int *ierror)
{
	...
	*ierror = MPI_Iscatter(sendbuf, *sendcount, c_sendtype, recvbuf, *recvcount, c_recvtype, *root, c_comm,
		&c_request);
	*request = PMPI_Request_c2f(c_request);
}

#pragma weak mpi_iscatter__ = pmpi_iscatter__
void pmpi_iscatter__(const void *sendbuf,
                     int *sendcount,
                     MPI_Fint *sendtype,
                     void *recvbuf,
                     int *recvcount,
...
void pmpi_iscatterv_(const void *sendbuf,
                     const int sendcounts[],
                     const int displs[],
                     MPI_Fint *sendtype,
                     void *recvbuf,
...
void pmpi_iscatterv__(const void *sendbuf,
                      const int sendcounts[],
                      const int displs[],
                      MPI_Fint *sendtype,
                      void *recvbuf,
...
void pmpi_isend_(const void *buf,
                 int *count,
                 MPI_Fint *datatype,
                 int *dest,
                 int *tag,
...
void pmpi_isend__(const void *buf,
                  int *count,
                  MPI_Fint *datatype,
                  int *dest,
                  int *tag,
...
void pmpi_issend_(const void *buf,
                  int *count,
                  MPI_Fint *datatype,
                  int *dest,
                  int *tag,
...
void pmpi_issend__(const void *buf,
                   int *count,
                   MPI_Fint *datatype,
                   int *dest,
                   int *tag,
...
void pmpi_keyval_create_(MPI_Copy_function *copy_fn,
...
void pmpi_keyval_create__(MPI_Copy_function *copy_fn,
                          MPI_Delete_function *delete_fn,
                          int *keyval,
                          void *extra_state,
                          int *ierror)
...
void pmpi_lookup_name_(const char *service_name CHAR_MIXED(size_service_name),
                       MPI_Fint *info,
...
void pmpi_lookup_name__(const char *service_name CHAR_MIXED(size_service_name),
                        MPI_Fint *info,
                        char *port_name CHAR_MIXED(size_port_name),
                        int *ierror CHAR_END(size_service_name)CHAR_END(size_port_name))
{
...
void pmpi_neighbor_allgather_(const void *sendbuf,
                              int *sendcount,
                              MPI_Fint *sendtype,
                              void *recvbuf,
                              int *recvcount,
...
void pmpi_neighbor_allgather__(const void *sendbuf,
                               int *sendcount,
                               MPI_Fint *sendtype,
                               void *recvbuf,
                               int *recvcount,
...
void pmpi_neighbor_allgather_init_(const void *sendbuf,
                                   int *sendcount,
                                   MPI_Fint *sendtype,
                                   void *recvbuf,
                                   int *recvcount,
                                   ...
                                   int *ierror)
{
	...
	*ierror = MPI_Neighbor_allgather_init(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		*recvcount,
...
void pmpi_neighbor_allgather_init__(const void *sendbuf,
                                    int *sendcount,
                                    MPI_Fint *sendtype,
                                    void *recvbuf,
                                    int *recvcount,
...
void pmpi_neighbor_allgatherv_(const void *sendbuf,
                               int *sendcount,
                               MPI_Fint *sendtype,
                               void *recvbuf,
                               const int recvcounts[],
...
void pmpi_neighbor_allgatherv__(const void *sendbuf,
                                int *sendcount,
                                MPI_Fint *sendtype,
                                void *recvbuf,
                                const int recvcounts[],
...
void pmpi_neighbor_allgatherv_init_(const void *sendbuf,
                                    int *sendcount,
                                    MPI_Fint *sendtype,
                                    void *recvbuf,
                                    const int recvcounts[],
                                    ...
                                    int *ierror)
{
	...
	*ierror = MPI_Neighbor_allgatherv_init(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		recvcounts,
...
void pmpi_neighbor_allgatherv_init__(const void *sendbuf,
                                     int *sendcount,
                                     MPI_Fint *sendtype,
                                     void *recvbuf,
                                     const int recvcounts[],
...
void pmpi_neighbor_alltoall_(const void *sendbuf,
                             int *sendcount,
                             MPI_Fint *sendtype,
                             void *recvbuf,
                             int *recvcount,
...
void pmpi_neighbor_alltoall__(const void *sendbuf,
                              int *sendcount,
                              MPI_Fint *sendtype,
                              void *recvbuf,
                              int *recvcount,
...
void pmpi_neighbor_alltoall_init_(const void *sendbuf,
                                  int *sendcount,
                                  MPI_Fint *sendtype,
                                  void *recvbuf,
                                  int *recvcount,
                                  ...
                                  int *ierror)
{
	...
	*ierror = MPI_Neighbor_alltoall_init(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		*recvcount,
...
void pmpi_neighbor_alltoall_init__(const void *sendbuf,
                                   int *sendcount,
                                   MPI_Fint *sendtype,
                                   void *recvbuf,
                                   int *recvcount,
...
void pmpi_neighbor_alltoallv_(const void *sendbuf,
                              const int sendcounts[],
                              const int sdispls[],
                              MPI_Fint *sendtype,
                              void *recvbuf,
                              ...
                              int *ierror)
{
	...
	*ierror = MPI_Neighbor_alltoallv(sendbuf,
		sendcounts,
		sdispls,
		c_sendtype,
		recvbuf,
...
void pmpi_neighbor_alltoallv__(const void *sendbuf,
                               const int sendcounts[],
                               const int sdispls[],
                               MPI_Fint *sendtype,
                               void *recvbuf,
...
void pmpi_neighbor_alltoallv_init_(const void *sendbuf,
                                   const int sendcounts[],
                                   const int sdispls[],
                                   MPI_Fint *sendtype,
                                   void *recvbuf,
                                   ...
                                   int *ierror)
{
	...
	*ierror = MPI_Neighbor_alltoallv_init(sendbuf,
		sendcounts,
		sdispls,
		c_sendtype,
		recvbuf,
...
void pmpi_neighbor_alltoallv_init__(const void *sendbuf,
                                    const int sendcounts[],
                                    const int sdispls[],
                                    MPI_Fint *sendtype,
                                    void *recvbuf,
...
void pmpi_neighbor_alltoallw_(const void *sendbuf,
                              const int sendcounts[],
                              const MPI_Aint sdispls[],
                              const MPI_Fint sendtypes[],
                              void *recvbuf,
                              ...
                              int *ierror)
{
	...
	*ierror = MPI_Neighbor_alltoallw(sendbuf,
		sendcounts,
		sdispls,
		c_sendtypes,
		recvbuf,
...
void pmpi_neighbor_alltoallw__(const void *sendbuf,
                               const int sendcounts[],
                               const MPI_Aint sdispls[],
                               const MPI_Fint sendtypes[],
                               void *recvbuf,
...
void pmpi_neighbor_alltoallw_init_(const void *sendbuf,
                                   const int sendcounts[],
                                   const MPI_Aint sdispls[],
                                   const MPI_Fint sendtypes[],
                                   void *recvbuf,
                                   ...
                                   int *ierror)
{
	...
	*ierror = MPI_Neighbor_alltoallw_init(sendbuf,
		sendcounts,
		sdispls,
		c_sendtypes,
		recvbuf,
...
void pmpi_neighbor_alltoallw_init__(const void *sendbuf,
                                    const int sendcounts[],
                                    const MPI_Aint sdispls[],
                                    const MPI_Fint sendtypes[],
                                    void *recvbuf,
...
void pmpi_pack_(const void *inbuf,
                int *incount,
                MPI_Fint *datatype,
                void *outbuf,
                int *outsize,
...
void pmpi_pack__(const void *inbuf,
                 int *incount,
                 MPI_Fint *datatype,
                 void *outbuf,
                 int *outsize,
...
void pmpi_pack_external_(const char datarep[] CHAR_MIXED(size_datarep),
                         const void *inbuf,
                         int *incount,
                         MPI_Fint *datatype,
                         void *outbuf,
...
void pmpi_pack_external__(const char datarep[] CHAR_MIXED(size_datarep),
                          const void *inbuf,
                          int *incount,
                          MPI_Fint *datatype,
                          void *outbuf,
...
void pmpi_pack_external_size_(const char datarep[] CHAR_MIXED(size_datarep),
                              int *incount,
                              MPI_Fint *datatype,
                              MPI_Aint *size,
                              int *ierror CHAR_END(size_datarep))
...
void pmpi_pack_external_size__(const char datarep[] CHAR_MIXED(size_datarep),
                               int *incount,
                               MPI_Fint *datatype,
                               MPI_Aint *size,
                               int *ierror CHAR_END(size_datarep))
...
void pmpi_publish_name_(const char *service_name CHAR_MIXED(size_service_name),
                        MPI_Fint *info,
                        const char *port_name CHAR_MIXED(size_port_name),
                        int *ierror CHAR_END(size_service_name)CHAR_END(size_port_name))
{
	/* MPI_Publish_name */
	char *tmp_service_name = NULL, *ptr_service_name = NULL;
...
void pmpi_publish_name__(const char *service_name CHAR_MIXED(size_service_name),
                         MPI_Fint *info,
                         const char *port_name CHAR_MIXED(size_port_name),
                         int *ierror CHAR_END(size_service_name)CHAR_END(size_port_name))
{
...
void pmpi_put_(const void *origin_addr,
               int *origin_count,
               MPI_Fint *origin_datatype,
               int *target_rank,
               MPI_Aint *target_disp,
...
void pmpi_put__(const void *origin_addr,
                int *origin_count,
                MPI_Fint *origin_datatype,
                int *target_rank,
                MPI_Aint *target_disp,
...
void pmpi_raccumulate_(const void *origin_addr,
                       int *origin_count,
                       MPI_Fint *origin_datatype,
                       int *target_rank,
                       MPI_Aint *target_disp,
                       ...
                       int *ierror)
{
	...
	*ierror = MPI_Raccumulate(origin_addr,
		*origin_count,
		c_origin_datatype,
		*target_rank,
		*target_disp,
...
void pmpi_raccumulate__(const void *origin_addr,
                        int *origin_count,
                        MPI_Fint *origin_datatype,
                        int *target_rank,
                        MPI_Aint *target_disp,
...
void pmpi_reduce_(const void *sendbuf,
                  void *recvbuf,
                  int *count,
                  MPI_Fint *datatype,
                  MPI_Fint *op,
...
void pmpi_reduce__(const void *sendbuf,
                   void *recvbuf,
                   int *count,
                   MPI_Fint *datatype,
                   MPI_Fint *op,
...
void pmpi_reduce_init_(const void *sendbuf,
                       void *recvbuf,
                       int *count,
                       MPI_Fint *datatype,
                       MPI_Fint *op,
...
void pmpi_reduce_init__(const void *sendbuf,
                        void *recvbuf,
                        int *count,
                        MPI_Fint *datatype,
                        MPI_Fint *op,
...
void pmpi_reduce_scatter_(const void *sendbuf,
                          void *recvbuf,
                          const int recvcounts[],
                          MPI_Fint *datatype,
                          MPI_Fint *op,
...
void pmpi_reduce_scatter__(const void *sendbuf,
                           void *recvbuf,
                           const int recvcounts[],
                           MPI_Fint *datatype,
                           MPI_Fint *op,
...
void pmpi_reduce_scatter_block_(const void *sendbuf,
                                void *recvbuf,
                                int *recvcount,
                                MPI_Fint *datatype,
                                MPI_Fint *op,
...
void pmpi_reduce_scatter_block__(const void *sendbuf,
                                 void *recvbuf,
                                 int *recvcount,
                                 MPI_Fint *datatype,
                                 MPI_Fint *op,
...
void pmpi_reduce_scatter_block_init_(const void *sendbuf,
                                     void *recvbuf,
                                     int *recvcount,
                                     MPI_Fint *datatype,
                                     MPI_Fint *op,
                                     ...
                                     int *ierror)
{
	...
	*ierror = MPI_Reduce_scatter_block_init(sendbuf, recvbuf, *recvcount, c_datatype, c_op, c_comm, c_info,
		&c_request);
	*request = PMPI_Request_c2f(c_request);
}

#pragma weak mpi_reduce_scatter_block_init__ = pmpi_reduce_scatter_block_init__
void pmpi_reduce_scatter_block_init__(const void *sendbuf,
                                      void *recvbuf,
                                      int *recvcount,
                                      MPI_Fint *datatype,
                                      MPI_Fint *op,
                                      ...
                                      int *ierror)
{
	...
	*ierror =
		MPI_Reduce_scatter_block_init(sendbuf, recvbuf, *recvcount, c_datatype, c_op, c_comm, c_info, &c_request);
	*request = PMPI_Request_c2f(c_request);
}

#pragma weak mpi_reduce_scatter_init_ = pmpi_reduce_scatter_init_
void pmpi_reduce_scatter_init_(const void *sendbuf,
                               void *recvbuf,
                               const int recvcounts[],
                               MPI_Fint *datatype,
                               MPI_Fint *op,
...
void pmpi_reduce_scatter_init__(const void *sendbuf,
                                void *recvbuf,
                                const int recvcounts[],
                                MPI_Fint *datatype,
                                MPI_Fint *op,
...
void pmpi_register_datarep_(const char *datarep CHAR_MIXED(size_datarep),
                            ...
                            int *ierror CHAR_END(size_datarep))
{
	...
	*ierror = MPI_Register_datarep(tmp_datarep,
		read_conversion_fn,
		write_conversion_fn,
...
void pmpi_register_datarep__(const char *datarep CHAR_MIXED(size_datarep),
                             MPI_Datarep_conversion_function *read_conversion_fn,
                             MPI_Datarep_conversion_function *write_conversion_fn,
                             MPI_Datarep_extent_function *dtype_file_extent_fn,
                             void *extra_state,
...
void pmpi_rget_(void *origin_addr,
                int *origin_count,
                MPI_Fint *origin_datatype,
                int *target_rank,
                MPI_Aint *target_disp,
...
void pmpi_rget__(void *origin_addr,
                 int *origin_count,
                 MPI_Fint *origin_datatype,
                 int *target_rank,
                 MPI_Aint *target_disp,
...
void pmpi_rget_accumulate_(const void *origin_addr,
                           int *origin_count,
                           MPI_Fint *origin_datatype,
                           void *result_addr,
                           int *result_count,
                           ...
                           int *ierror)
{
	...
	*ierror = MPI_Rget_accumulate(origin_addr,
		*origin_count,
		c_origin_datatype,
		result_addr,
		*result_count,
...
void pmpi_rget_accumulate__(const void *origin_addr,
                            int *origin_count,
                            MPI_Fint *origin_datatype,
                            void *result_addr,
                            int *result_count,
...
void pmpi_rput_(const void *origin_addr,
                int *origin_count,
                MPI_Fint *origin_datatype,
                int *target_rank,
                MPI_Aint *target_disp,
...
void pmpi_rput__(const void *origin_addr,
                 int *origin_count,
                 MPI_Fint *origin_datatype,
                 int *target_rank,
                 MPI_Aint *target_disp,
...
void pmpi_rsend_init_(const void *buf,
                      int *count,
                      MPI_Fint *datatype,
                      int *dest,
                      int *tag,
...
void pmpi_rsend_init__(const void *buf,
                       int *count,
                       MPI_Fint *datatype,
                       int *dest,
                       int *tag,
...
void pmpi_scan_(const void *sendbuf,
                void *recvbuf,
                int *count,
                MPI_Fint *datatype,
                MPI_Fint *op,
...
void pmpi_scan__(const void *sendbuf,
                 void *recvbuf,
                 int *count,
                 MPI_Fint *datatype,
                 MPI_Fint *op,
...
void pmpi_scan_init_(const void *sendbuf,
                     void *recvbuf,
                     int *count,
                     MPI_Fint *datatype,
                     MPI_Fint *op,
...
void pmpi_scan_init__(const void *sendbuf,
                      void *recvbuf,
                      int *count,
                      MPI_Fint *datatype,
                      MPI_Fint *op,
...
void pmpi_scatter_(const void *sendbuf,
                   int *sendcount,
                   MPI_Fint *sendtype,
                   void *recvbuf,
                   int *recvcount,
...
void pmpi_scatter__(const void *sendbuf,
                    int *sendcount,
                    MPI_Fint *sendtype,
                    void *recvbuf,
                    int *recvcount,
...
void pmpi_scatter_init_(const void *sendbuf,
                        int *sendcount,
                        MPI_Fint *sendtype,
                        void *recvbuf,
                        int *recvcount,
                        ...
                        int *ierror)
{
	...
	*ierror = MPI_Scatter_init(sendbuf,
		*sendcount,
		c_sendtype,
		recvbuf,
		*recvcount,
...
void pmpi_scatter_init__(const void *sendbuf,
                         int *sendcount,
                         MPI_Fint *sendtype,
                         void *recvbuf,
                         int *recvcount,
...
void pmpi_scatterv_(const void *sendbuf,
                    const int sendcounts[],
                    const int displs[],
                    MPI_Fint *sendtype,
                    void *recvbuf,
...
void pmpi_scatterv__(const void *sendbuf,
                     const int sendcounts[],
                     const int displs[],
                     MPI_Fint *sendtype,
                     void *recvbuf,
...
void pmpi_scatterv_init_(const void *sendbuf,
                         const int sendcounts[],
                         const int displs[],
                         MPI_Fint *sendtype,
                         void *recvbuf,
                         ...
                         int *ierror)
{
	...
	*ierror = MPI_Scatterv_init(sendbuf,
		sendcounts,
		displs,
		c_sendtype,
		recvbuf,
...
void pmpi_scatterv_init__(const void *sendbuf,
                          const int sendcounts[],
                          const int displs[],
                          MPI_Fint *sendtype,
                          void *recvbuf,
...
void pmpi_send_init_(const void *buf,
                     int *count,
                     MPI_Fint *datatype,
                     int *dest,
                     int *tag,
...
void pmpi_send_init__(const void *buf,
                      int *count,
                      MPI_Fint *datatype,
                      int *dest,
                      int *tag,
...
void pmpi_sendrecv_(const void *sendbuf,
                    int *sendcount,
                    MPI_Fint *sendtype,
                    int *dest,
                    int *sendtag,
...
void pmpi_sendrecv__(const void *sendbuf,
                     int *sendcount,
                     MPI_Fint *sendtype,
                     int *dest,
                     int *sendtag,
...
void pmpi_sendrecv_replace_(void *buf,
                            int *count,
                            MPI_Fint *datatype,
                            int *dest,
                            int *sendtag,
...
void pmpi_sendrecv_replace__(void *buf,
                             int *count,
                             MPI_Fint *datatype,
                             int *dest,
                             int *sendtag,
...
void pmpi_ssend_init_(const void *buf,
                      int *count,
                      MPI_Fint *datatype,
                      int *dest,
                      int *tag,
...
void pmpi_ssend_init__(const void *buf,
                       int *count,
                       MPI_Fint *datatype,
                       int *dest,
                       int *tag,
...
void pmpi_testsome_(int *incount,
...
void pmpi_testsome__(int *incount,
                     MPI_Fint array_of_requests[],
                     int *outcount,
                     int array_of_indices[],
                     MPI_Status array_of_statuses[],
...
void pmpi_type_create_darray_(int *size,
                              int *rank,
                              int *ndims,
...
void pmpi_type_create_darray__(int *size,
                               int *rank,
                               int *ndims,
                               const int array_of_gsizes[],
                               const int array_of_distribs[],
...
void pmpi_type_create_hindexed_(int *count,
...
void pmpi_type_create_hindexed__(int *count,
                                 const int array_of_blocklengths[],
                                 const MPI_Aint array_of_displacements[],
                                 MPI_Fint *oldtype,
                                 MPI_Fint *newtype,
...
void pmpi_type_create_hindexed_block_(int *count,
                                      int *blocklength,
                                      const MPI_Aint array_of_displacements[],
                                      MPI_Fint *oldtype,
                                      MPI_Fint *newtype,
...
void pmpi_type_create_hindexed_block__(int *count,
                                       int *blocklength,
                                       const MPI_Aint array_of_displacements[],
                                       MPI_Fint *oldtype,
                                       MPI_Fint *newtype,
...
void pmpi_type_hvector_(int *count,
                        int *blocklength,
                        MPI_Aint *stride,
                        MPI_Fint *oldtype,
                        MPI_Fint *newtype,
...
void pmpi_type_hvector__(int *count,
                         int *blocklength,
                         MPI_Aint *stride,
                         MPI_Fint *oldtype,
                         MPI_Fint *newtype,
...
void pmpi_type_create_hvector_(int *count,
                               int *blocklength,
                               MPI_Aint *stride,
                               MPI_Fint *oldtype,
                               MPI_Fint *newtype,
...
void pmpi_type_create_hvector__(int *count,
                                int *blocklength,
                                MPI_Aint *stride,
                                MPI_Fint *oldtype,
                                MPI_Fint *newtype,
...
void pmpi_type_create_indexed_block_(int *count,
                                     int *blocklength,
...
void pmpi_type_create_indexed_block__(int *count,
                                      int *blocklength,
                                      const int array_of_displacements[],
                                      MPI_Fint *oldtype,
                                      MPI_Fint *newtype,
...
void pmpi_type_create_keyval_(MPI_Type_copy_attr_function *type_copy_attr_fn,
...
void pmpi_type_create_keyval__(MPI_Type_copy_attr_function *type_copy_attr_fn,
                               MPI_Type_delete_attr_function *type_delete_attr_fn,
                               int *type_keyval,
                               void *extra_state,
                               int *ierror)
...
void pmpi_type_create_struct_(int *count,
                              const int array_of_blocklengths[],
                              const MPI_Aint array_of_displacements[],
...
void pmpi_type_create_struct__(int *count,
                               const int array_of_blocklengths[],
                               const MPI_Aint array_of_displacements[],
                               const MPI_Fint array_of_types[],
                               MPI_Fint *newtype,
...
void pmpi_type_create_subarray_(int *ndims,
...
void pmpi_type_create_subarray__(int *ndims,
                                 const int array_of_sizes[],
                                 const int array_of_subsizes[],
                                 const int array_of_starts[],
                                 int *order,
...
void pmpi_type_get_contents_(MPI_Fint *datatype,
                             int *max_integers,
                             int *max_addresses,
                             int *max_datatypes,
                             ...
                             int *ierror)
{
	...
	*ierror = MPI_Type_get_contents(c_datatype,
		*max_integers,
		*max_addresses,
		*max_datatypes,
		array_of_integers,
...
void pmpi_type_get_contents__(MPI_Fint *datatype,
                              int *max_integers,
                              int *max_addresses,
                              int *max_datatypes,
                              int array_of_integers[],
...
void pmpi_type_get_name_(MPI_Fint *datatype,
...
void pmpi_type_get_name__(MPI_Fint *datatype,
                          char *type_name CHAR_MIXED(size_type_name),
                          int *resultlen,
                          int *ierror CHAR_END(size_type_name))
{
...
void pmpi_type_indexed_(int *count,
                        const int array_of_blocklengths[],
                        const int array_of_displacements[],
                        MPI_Fint *oldtype,
                        MPI_Fint *newtype,
...
void pmpi_type_indexed__(int *count,
                         const int array_of_blocklengths[],
                         const int array_of_displacements[],
                         MPI_Fint *oldtype,
                         MPI_Fint *newtype,
...
void pmpi_type_set_name_(MPI_Fint *datatype, const char *type_name CHAR_MIXED(size_type_name),
                         int *ierror CHAR_END(size_type_name))
{
	/* MPI_Type_set_name */
	MPI_Datatype c_datatype = PMPI_Type_f2c(*datatype);
...
void pmpi_type_set_name__(MPI_Fint *datatype, const char *type_name CHAR_MIXED(size_type_name),
                          int *ierror CHAR_END(size_type_name))
{
	/* MPI_Type_set_name */
	MPI_Datatype c_datatype = PMPI_Type_f2c(*datatype);
...
void pmpi_unpack_(const void *inbuf,
                  int *insize,
                  int *position,
                  void *outbuf,
                  int *outcount,
...
void pmpi_unpack__(const void *inbuf,
                   int *insize,
                   int *position,
                   void *outbuf,
                   int *outcount,
...
void pmpi_unpack_external_(const char datarep[] CHAR_MIXED(size_datarep),
                           const void *inbuf,
                           MPI_Aint *insize,
                           MPI_Aint *position,
                           void *outbuf,
...
void pmpi_unpack_external__(const char datarep[] CHAR_MIXED(size_datarep),
                            const void *inbuf,
                            MPI_Aint *insize,
                            MPI_Aint *position,
                            void *outbuf,
...
void pmpi_unpublish_name_(const char *service_name CHAR_MIXED(size_service_name),
                          MPI_Fint *info,
                          const char *port_name CHAR_MIXED(size_port_name),
                          int *ierror CHAR_END(size_service_name)CHAR_END(size_port_name))
{
...
void pmpi_unpublish_name__(const char *service_name CHAR_MIXED(size_service_name),
                           MPI_Fint *info,
                           const char *port_name CHAR_MIXED(size_port_name),
                           int *ierror CHAR_END(size_service_name)CHAR_END(size_port_name))
{
...
void pmpi_waitsome_(int *incount,
                    MPI_Fint array_of_requests[],
                    int *outcount,
                    int array_of_indices[],
                    MPI_Status array_of_statuses[],
...
void pmpi_waitsome__(int *incount,
                     MPI_Fint array_of_requests[],
                     int *outcount,
                     int array_of_indices[],
                     MPI_Status array_of_statuses[],
...
void pmpi_win_allocate_(MPI_Aint *size,
                        int *disp_unit,
                        MPI_Fint *info,
                        MPI_Fint *comm,
                        void *baseptr,
...
void pmpi_win_allocate__(MPI_Aint *size,
                         int *disp_unit,
                         MPI_Fint *info,
                         MPI_Fint *comm,
                         void *baseptr,
...
void pmpi_win_allocate_shared_(MPI_Aint *size,
                               int *disp_unit,
                               MPI_Fint *info,
                               MPI_Fint *comm,
                               void *baseptr,
...
void pmpi_win_allocate_shared__(MPI_Aint *size,
                                int *disp_unit,
                                MPI_Fint *info,
                                MPI_Fint *comm,
                                void *baseptr,
...
void pmpi_win_create_(void *base,
                      MPI_Aint *size,
                      int *disp_unit,
                      MPI_Fint *info,
                      MPI_Fint *comm,
...
void pmpi_win_create__(void *base,
                       MPI_Aint *size,
                       int *disp_unit,
                       MPI_Fint *info,
                       MPI_Fint *comm,
...
void pmpi_win_create_keyval_(MPI_Win_copy_attr_function *win_copy_attr_fn,
...
void pmpi_win_create_keyval__(MPI_Win_copy_attr_function *win_copy_attr_fn,
                              MPI_Win_delete_attr_function *win_delete_attr_fn,
                              int *win_keyval,
                              void *extra_state,
                              int *ierror)
...
void pmpi_win_get_name_(MPI_Fint *win, char *win_name CHAR_MIXED(size_win_name), int *resultlen,
...
void pmpi_win_get_name__(MPI_Fint *win, char *win_name CHAR_MIXED(size_win_name), int *resultlen,
                         int *ierror CHAR_END(size_win_name))
{
	/* MPI_Win_get_name */
	MPI_Win c_win = PMPI_Win_f2c(*win);
...
void pmpi_win_set_name_(MPI_Fint *win, const char *win_name CHAR_MIXED(size_win_name),
                        int *ierror CHAR_END(size_win_name))
{
	/* MPI_Win_set_name */
	MPI_Win c_win = PMPI_Win_f2c(*win);
...
void pmpi_win_set_name__(MPI_Fint *win, const char *win_name CHAR_MIXED(size_win_name),
                         int *ierror CHAR_END(size_win_name))
{
	/* MPI_Win_set_name */
	MPI_Win c_win = PMPI_Win_f2c(*win);
...
void pmpi_session_get_nth_pset_(MPI_Fint *session,
                                MPI_Fint *info,
                                MPI_Fint *n,
                                MPI_Fint *pset_len,
                                char *pset_name CHAR_MIXED(size_string),
                                int *ierror CHAR_END(size_string))
{
	MPI_Session c_session = PMPI_Session_f2c(*session);
	MPI_Info    c_info    = PMPI_Info_f2c(*info);
...
void pmpi_session_get_nth_pset__(MPI_Fint *session,
                                 MPI_Fint *info,
                                 MPI_Fint *n,
                                 MPI_Fint *pset_len,
                                 char *pset_name CHAR_MIXED(size_string),
...
void pmpi_group_from_session_pset_(MPI_Fint *session,
                                   const char *pset_name CHAR_MIXED(size_string),
                                   MPI_Fint *newgroup,
                                   int *ierror CHAR_END(size_string))
{
	MPI_Session c_session = PMPI_Session_f2c(*session);
...
void pmpi_group_from_session_pset__(MPI_Fint *session,
                                    const char *pset_name CHAR_MIXED(size_string),
                                    MPI_Fint *newgroup,
                                    int *ierror CHAR_END(size_string))
{
...
void pmpi_comm_create_from_group_(MPI_Fint *group,
                                  const char * stringtag CHAR_MIXED(size_string),
                                  MPI_Fint *info,
                                  MPI_Fint *errhandler,
                                  MPI_Fint *newcomm,
                                  int *ierror CHAR_END(size_string))
...
void pmpi_comm_create_from_group__(MPI_Fint *group,
                                   const char * stringtag CHAR_MIXED(size_string),
                                   MPI_Fint *info,
                                   MPI_Fint *errhandler,
                                   MPI_Fint *newcomm,
...
void pmpi_session_get_pset_info_(MPI_Fint *session,
                                 const char *pset_name CHAR_MIXED(size_string),
                                 MPI_Fint *info,
                                 int * ierror CHAR_END(size_string))
{
...
void pmpi_session_get_pset_info__(MPI_Fint *session,
                                  const char *pset_name CHAR_MIXED(size_string),
                                  MPI_Fint *info,
                                  int * ierror CHAR_END(size_string))
{
