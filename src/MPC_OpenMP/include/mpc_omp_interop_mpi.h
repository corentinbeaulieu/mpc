...
__request_get_test(mpix_progress_info_t *infos)
{
	...
	if (infos->partition != MPIX_PARTITION_NONE)
	{
		return __request_testpartition;
	}
...
{
	if (mpc_thread_mpi_omp_wait(n, requests, NULL, status, MPIX_PARTITION_NONE))
	{
		return 0;
	}
...
{
	if (mpc_thread_mpi_omp_wait(1, request, NULL, status, partition))
	{
		return 0;
	}
