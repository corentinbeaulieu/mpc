...
static inline void __entry_init(struct mpc_common_init_entry *entry,
                                char *callback_name,
                                void (*callback)(),
                                int priority)
{
...
static inline void __list_trigger(struct mpc_common_init_list *list)
{
	...
	for (i = 0 ; i < list->entry_count; i++)
	{
		...
		{
			mpc_common_debug("INIT : Triggering %s - %s (%d)",
				list->name,
				list->entries[i].name,
				list->entries[i].priority);
		}
...
static inline void __list_add_callback(struct mpc_common_init_list *list,
                                       char *callback_name,
                                       void (*callback)(),
                                       int priority)
{
	...
	{
		mpc_common_debug_fatal("Maximum number of entries reached when adding callback '%s' to '%s'",
			callback_name,
			list->name);
	}
