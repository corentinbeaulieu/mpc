...
{
	return (mpc_mempool_elem_t *)mp->data->malloc_func(mp->data->elem_size + sizeof(mpc_mempool_elem_t));
}
...
static void *mpc_mempool_chunk_elems(mpc_mempool_data_t *data, mpc_mempool_chunk_t *chunk)
{
	...
	uintptr_t chunk_header = (uintptr_t)chunk + sizeof(mpc_mempool_chunk_t) + sizeof(mpc_mempool_elem_t);

	chunk_header_padding = mpc_common_padding(chunk_header, data->alignment);
	return (void *)(chunk_header + (intptr_t)chunk_header_padding - sizeof(mpc_mempool_elem_t));
}
...
static mpc_mempool_elem_t *mpc_mempool_chunk_elem(mpc_mempool_data_t *mp,
                                                  ...
                                                  int index)
{
	return (mpc_mempool_elem_t *)((uintptr_t)chunk->elems + (intptr_t)(index * mpc_mempool_elem_real_size(mp)));
}
