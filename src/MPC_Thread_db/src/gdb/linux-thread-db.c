...
/*
 * FIXME: Most of the included headers are missing, this file no longer compiles
 */
...
static td_err_e (*td_ta_map_id2thr_p)(const td_thragent_t *ta, thread_t pt, td_thrhandle_t *__th);
static td_err_e (*td_ta_map_lwp2thr_p)(const td_thragent_t *ta, lwpid_t lwpid, td_thrhandle_t *th);
static td_err_e (*td_ta_thr_iter_p)(const td_thragent_t *ta, td_thr_iter_f *callback, void *cbdata_p,
                                    td_thr_state_e state, int ti_pri,
                                    sigset_t *ti_sigmask_p, unsigned int ti_user_flags);
static td_err_e (*td_ta_event_addr_p)(const td_thragent_t *ta, td_event_e event, td_notify_t *ptr);
static td_err_e (*td_ta_set_event_p)(const td_thragent_t *ta, td_thr_events_t *event);
static td_err_e (*td_ta_event_getmsg_p)(const td_thragent_t *ta, td_event_msg_t *msg);

static td_err_e (*td_thr_validate_p)(const td_thrhandle_t *th);
static td_err_e (*td_thr_get_info_p)(const td_thrhandle_t *th, td_thrinfo_t *infop);
static td_err_e (*td_thr_event_enable_p)(const td_thrhandle_t *th, int event);

static td_err_e (*td_thr_tls_get_addr_p)(const td_thrhandle_t *th, void *map_address, size_t offset, void **address);
/** ** **/
static td_err_e (*td_thr_getfpregs_p)(const td_thrhandle_t *th_p, prfpregset_t *fpregset);
static td_err_e (*td_thr_setfpregs_p)(const td_thrhandle_t *th_p, const prfpregset_t *fpregset);
static td_err_e (*td_thr_getgregs_p)(const td_thrhandle_t *th_p, prgregset_t regset);
static td_err_e (*td_thr_setgregs_p)(const td_thrhandle_t *th_p, const prgregset_t regset);
/** **/
...
static void attach_thread(ptid_t ptid, const td_thrhandle_t *th_p, const td_thrinfo_t *ti_p);
...
thread_db_load(void)
{
	...
	if (tdb_debug)
	{
		/*mandatory functions, not dlopened by GDB otherwise*/
		td_thr_getgregs_p = verbose_dlsym(handle, "td_thr_getgregs");
		tdb_debug        &= (td_thr_getgregs_p != NULL);
...
/* Worker bee for the "info tdb-thread" command.  This is a callback
...
static int info_cb(const td_thrhandle_t *th, void *arg)
{
	...
	if (ret == TD_OK)
	{
		printf_filtered("#%d) %s thread #%p, lwp %d, ",
			*no, ti.ti_type == TD_THR_SYSTEM ? "system" : "user  ",
			(void *)ti.ti_tid, ti.ti_lid);
		switch (ti.ti_state)
		{
		default:
		case TD_THR_UNKNOWN:
		...
		if (ti.ti_startfunc != 0)
		{
			...
			{
				printf_filtered("   startfunc: 0x%s\n", paddr((CORE_ADDR)ti.ti_startfunc));
			}
		}
...
/* List some state about each TDB user-level thread in the
 * inferior.  */
...
static void info_tdbthreads(char *args, int from_tty)
{
	...
	td_ta_thr_iter_p(thread_agent, info_cb, &no,
		TD_THR_ANY_STATE, TD_THR_LOWEST_PRIORITY,
		TD_SIGNO_MASK, TD_THR_ANY_USER_FLAGS);
}
...
static void
set_tdb_verbosity(char *args, int from_tty, struct cmd_list_element *c)
{
...
static void
show_tdb_verbosity(struct ui_file *file, int from_tty,
