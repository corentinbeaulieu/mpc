...
#include "sctk_lib_thread_db_tests.h"
	...
	int test_td_consistency(void)
	{
		tdb_log("test_td_consistency");
		{
			...
			{
				assert(TD_DBERR == td_ta_thr_iter(test_data.ta, test_td_err_callback, NULL, TD_THR_ANY_STATE, 0, NULL,
					0));
			}
		}
		{
			...
			if ((TD_OK == td_ta_get_nthreads(test_data.ta, &nthread))
			    && (TD_OK == td_ta_thr_iter(test_data.ta,
					test_td_consistency_callback,
					&iter,
					TD_THR_ANY_STATE,
					0,
					NULL,
