 ...
 */
void mpc_topology_render_text(int os_pu, int os_master_pu,
                              int task_id, int vp, __UNUSED__ int rank_open_mp,
                              int *min_idex, int pid)
{
	/*access global topo*/
	...
	else
	{
		...
		if (f_textual != NULL)
		{
			fprintf(f_textual, "%d %d %d %d %d %d %d\n", os_pu, vp, task_id, min_idex[0], min_idex[1], min_idex[2],
				pid);
			fclose(f_textual);
		}
	}
...
/* fill thread placement information in file to communicate between processes of the same node for graphic placement
 * option */
...
static void print_children(hwloc_topology_t hwtopology, hwloc_obj_t obj,
                           int depth, struct sctk_text_option_s *tab_option,
                           int num_os, unsigned int higher_logical,
                           int lower_logical, const char *HostName,
                           FILE *f, int __UNUSED__ ind_child, int last_arity)
{
	char     string[128];
	char     string_mpc[128];
	unsigned i;
	...
	if (already_beginning_done)
	{
		...
		/* if ancestor of the first pu booked */
			...
			if (hwloc_get_ancestor_obj_by_type(hwtopology, obj->type,
			lower_index_obj_pu)->logical_index == obj->logical_index
			    && obj->type != HWLOC_OBJ_MACHINE && obj->type != HWLOC_OBJ_NODE && obj->type != HWLOC_OBJ_SOCKET
	...
	if (obj->type == type)
	{
		for (k = 0; k < num_os; k++)
		{
			...
			{
				sprintf(string_mpc,
					"Virtual Processor %d | rank MPI %d | Policy : compact %d, scatter %d, balanced %d | tid %d",
					tab_option->vp_tab[k],
					tab_option->rank_mpi[k],
					tab_option->compact_tab[k],
					tab_option->scatter_tab[k],
					tab_option->balanced_tab[k],
					tab_option->pid_tab[k]);
				if (last_arity == 1)
				{
...
void _mpc_topology_render_init(void)
{
	/*graphical option*/
	if (mpc_common_get_flags()->enable_topology_graphic_placement
	    || mpc_common_get_flags()->enable_topology_text_placement)
	{
...
void topology_graph_enable_graphic_placement(void)
{
	...
	if (cluster != NULL)
	{
		...
		{ // TODO one among each nodes
			fprintf(stdout,
				"/* --graphic-placement : \n.xml dated file has been generated for"
				" each compute node to visualise topology and thread placement with their infos.\n"
				"You can use the command \"lstopo -i file.xml\" to visualise graphically */\n");
			fprintf(stdout,
				"\n/* .xml legend : one color per MPI task."
				" White text policy means the thread is master of a MPI task in MPC */\n\n");
			fflush(stdout);
			remove(placement_txt);
		}
	}
	...
	if (f_textual != NULL)
	{
		...
		if (f != NULL)
		{
			...
			{ // TODO one proc among nodes
				fprintf(stdout,
					"/* --text-placement : \n.txt dated file has been generated for each compute node to visualise topology"
					" and thread placement with their infos. */\n");
				fflush(stdout);
			}
			print_children(
